...
export default function GooeyNav({
  ...
  const makeParticles = (element: HTMLElement) => {
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      setTimeout(() => {
        ...
        particle.style.setProperty("--color", `var(--color-${p.color}, #10b981)`);
  ...
  return (
    <div
      className={`gooey-nav-container rounded-full border border-rule/80 bg-white/70 shadow-xs backdrop-blur-md dark:border-dark-border dark:bg-dark-card/70 ${className}`}
      ref={containerRef}
    >
      <nav>
        <ul ref={navRef} className="text-mute dark:text-dark-mute">
          {items.map((item, index) => (
            <li
              key={index}
              className={`transition-colors ${activeIndex === index ? "active font-semibold" : "hover:text-ink dark:hover:text-dark-ink"}`}
