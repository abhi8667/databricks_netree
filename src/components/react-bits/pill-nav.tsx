...
export default function PillNav({
  ...
  ease = "power3.easeOut",
  baseColor = "#059669",
  pillColor = "var(--pill-bg-dynamic, #ffffff)",
  hoveredPillTextColor = "#ffffff",
  pillTextColor = "var(--pill-text-dynamic, #047857)",
  onMobileMenuClick,
  ...
  return (
    <div className="pill-nav-container">
      <nav className={`pill-nav ${className}`} aria-label="Primary" style={cssVars}>
                ...
                >
                  ...
                  <span className="label-stack">
                    <span className="pill-label flex items-center gap-1.5">
                      ...
                      {item.count ? (
                        <span className="pill-count-badge">{item.count}</span>
                      ) : null}
                    </span>
                    <span className="pill-label-hover flex items-center gap-1.5" aria-hidden="true">
                      <span>{item.label}</span>
                      {item.count ? (
                        <span className="pill-count-badge bg-white/25 text-white">
                          {item.count}
                        </span>
                      ) : null}
                    </span>
        ...
        <button
          type="button"
          className="mobile-menu-button pill-mobile-only"
      ...
      <div className="mobile-menu-popover pill-mobile-only" ref={mobileMenuRef} style={cssVars}>
        <ul className="mobile-menu-list">
          {items.map((item, i) => (
            <li key={item.href || `mobile-item-${i}`}>
              ...
              >
                <span>{item.label}</span>
                {item.count ? <span className="pill-count-badge">{item.count}</span> : null}
              </Link>
            </li>
          ))}
        </ul>
